...
echo "SETDEVELOPERS - DevOps.CiCd.Manager"
echo "Starting..."
docker run --quiet --privileged --security-opt seccomp=unconfined --security-opt apparmor=unconfined --cap-add CAP_SYS_ADMIN --env-file env.list $DEVOPS_DOCKER_REGISTRY_URL/$DEVOPS_DOCKER_REGISTRY_IMAGE_NAME:$DEVOPS_DOCKER_REGISTRY_IMAGE_TAG