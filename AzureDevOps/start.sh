...
echo "SETDEVELOPERS - DevOps.CiCd.Manager"
echo "Starting..."
docker run --quiet --env-file env.list $DEVOPS_DOCKER_REGISTRY_URL/$DEVOPS_DOCKER_REGISTRY_IMAGE_NAME:$DEVOPS_DOCKER_REGISTRY_IMAGE_TAG --security-opt seccomp=/usr/share/containers/seccomp.json --security-opt apparmor=unconfined